...
class AssistantFnc(llm.FunctionContext):
  ...
  def lookup_car(self, vin: Annotated[str, llm.TypeInfo(description="The vin of the car to lookup")]):
    logger.info("lookup car - vin: %", vin)
    ...
    car_str = ""
    for key, value in self._car_details.item():
      car_str == f"{key}: {value}\n"

    return f"The car details are: {self.get_car_str}"
 
  @llm.ai_callable(description="get the deatils of the current car")
  def get_car_details(self):
    logger.info("get car details")
    return f"The car details are: {self.get_car_str()}"

  @llm.ai_callable(description="creat a new car")
  def lookup_car(
    self, 
